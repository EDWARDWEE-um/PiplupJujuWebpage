...
import { useGamification, type Reward } from "@/contexts/gamification-context"
...
export default function RewardsDisplay() {
  ...
  const handleClaimReward = (code: string) => {
    const success = claimReward(code)
    if (!success) {
      toast({
        title: "Error",
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Badge variant="outline" className="px-2 py-1">
          {state.rewards.length} Available
        </Badge>
      </div>
          ...
          {filteredRewards.length > 0 ? (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filteredRewards.map((reward, index) => (
                <RewardCard key={index} reward={reward} onClaim={handleClaimReward} />
              ))}
            </div>
          ) : (
...
function RewardCard({ reward, onClaim }: { reward: Reward; onClaim: (code: string) => void }) {
  const getIcon = (type: string) => {
    switch (type) {
      case "discount":
        return <Tag className="h-5 w-5" />
  ...
  return (
    <Card className="overflow-hidden transition-all">
      ...
      <CardContent className="pb-2">
        {reward.code && <div className="bg-muted p-2 rounded text-center font-mono text-sm">{reward.code}</div>}
      </CardContent>
      <CardFooter className="flex justify-between items-center">
        <div className="text-xs text-muted-foreground">{formatExpiry(reward.expiresAt)}</div>
        {reward.code && (
          <Button size="sm" onClick={() => onClaim(reward.code!)}>
            Claim
          </Button>
        )}
      </CardFooter>
